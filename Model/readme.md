Here’s a copy-paste “state + next steps” note you can drop into a new chat anytime.
...
1. Ensure training/inference uses the sparse path (verify `time_attention_type` is set in the actual config at runtime).
2. Make sure `context_length` is not silently truncating in dataset/pipeline/trainer.
3. Confirm `num_output_patches` is correct for your training objective (future tokens must be the last tokens).
4. Decide batch size + grad accumulation settings for long contexts (e.g., 8k/16k) and check memory.
5. Add quick sanity tests:
...
If you paste this into a new chat, the next assistant should pick up from “Next stage” and proceed with fine-tuning wiring and long-context training details.
